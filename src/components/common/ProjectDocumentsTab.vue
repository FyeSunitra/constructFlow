...
<template>
    <div>
        <div v-if="documents.length" class="bg-white rounded-xl overflow-hidden" style="border:0.5px solid #E3E1D8">
            <div v-for="(doc, i) in documents" :key="doc.id"
                class="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 transition-colors"
                :style="i < documents.length - 1 ? 'border-bottom:0.5px solid #F7F6F2' : ''">
                <div class="flex-shrink-0 w-8 h-8 rounded-lg flex items-center justify-center"
                    :style="{ background: (TYPE_COLOR[fileType(doc.name)] ?? '#888780') + '18' }">
                    <Icon :icon="TYPE_ICON[fileType(doc.name)] ?? 'lucide:file'"
                        :style="{ width: '15px', height: '15px', color: TYPE_COLOR[fileType(doc.name)] ?? '#888780' }" />
                </div>

                <div class="flex-1 min-w-0">
                    <div class="text-sm font-medium text-gray-800 truncate">{{ doc.name }}</div>
                    <div class="text-xs text-gray-400 mt-0.5">
                        อัปโหลด {{ formatDate(doc.created_at) }} · {{ fileType(doc.name) }}
                    </div>
                </div>

...
import {
    type Template, type TemplateForm, type PhaseDraft,
} from '@/types/template'
import TemplatePhaseEditor from '@/components/common/TemplatePhaseEditor.vue'
import { useBreakpoint } from '@/composables/useBreakpoint'
import { buildPayload, emptyForm, emptyPhaseDraft, templateToForm } from '@/lib/template'
...
const modalTitle = computed(() => isEdit.value ? 'แก้ไข Template' : 'สร้าง Template ใหม่')
...
const rules: FormRules = {
    name: [{ required: true, trigger: ['blur', 'input'], message: 'กรุณากรอกชื่อ Template' }],
}
    ...
    <NModal :show="show" preset="card" :title="modalTitle" :style="modalStyle" :mask-closable="false"
        ...
        <NForm ref="formRef" :model="form" :rules="rules" label-placement="top" require-mark-placement="right-hanging"
            ...
            <NFormItem label="ชื่อ Template" path="name">
                <NInput v-model:value="form.name" placeholder="ระบุชื่อ Template" />
            </NFormItem>

            <NFormItem label="คำอธิบาย" path="description" style="margin-bottom:8px">
                <NInput v-model:value="form.description" type="textarea" :rows="2" placeholder="ระบุคำอธิบาย (ถ้ามี)" />
            </NFormItem>

            <div class="text-[10px] font-semibold text-gray-400 uppercase tracking-wider mb-3 pb-2"
                style="border-bottom:0.5px solid #F1EFE8">
                กำหนด Phase และ Checkpoint
            </div>

            <div class="flex flex-col gap-3">
                <TemplatePhaseEditor v-for="(phase, i) in form.phases" :key="i" :phase="phase" :index="i"
                    :can-delete="form.phases.length > 1" @update:phase="updatePhase(i, $event)"

...
import { NInput, NInputNumber, NDatePicker, NButton, NPopconfirm } from 'naive-ui'
...
const startTs = computed({
    get: () => props.phase.start_date ? new Date(props.phase.start_date).getTime() : null,
    set: (v) => update({ start_date: v ? new Date(v).toISOString().split('T')[0] : '' }),
})
const endTs = computed({
    get: () => props.phase.end_date ? new Date(props.phase.end_date).getTime() : null,
    set: (v) => update({ end_date: v ? new Date(v).toISOString().split('T')[0] : '' }),
})
</script>
    ...
    <div class="rounded-xl overflow-hidden" style="border:0.5px solid #E3E1D8">
        ...
        <div class="p-3 flex flex-col gap-3">
            ...
            <div class="grid grid-cols-2 gap-2">
                <div>
                    <div class="text-[10px] text-gray-400 mb-1">งบประมาณ Phase (บาท)</div>
                    <NInputNumber :value="phase.budget_estimate" size="small" :min="0" :show-button="false"
                        placeholder="0" style="width:100%" @update:value="update({ budget_estimate: $event ?? 0 })" />
                </div>
                <div>
                    <div class="text-[10px] text-gray-400 mb-1">% จากงบรวม</div>
                    <div class="h-8 flex items-center px-3 rounded-lg text-sm text-gray-500"
            ...
            <div class="grid grid-cols-2 gap-2">
                <div>
                    <div class="text-[10px] text-gray-400 mb-1">วันเริ่ม Phase</div>
                    <NDatePicker v-model:value="startTs" type="date" size="small" placeholder="เลือกวันที่"
                        style="width:100%" clearable />
                </div>
                <div>
                    <div class="text-[10px] text-gray-400 mb-1">วันสิ้นสุด Phase</div>
                    <NDatePicker v-model:value="endTs" type="date" size="small" placeholder="เลือกวันที่"
                        style="width:100%" clearable />
                </div>
            </div>

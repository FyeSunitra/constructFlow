...
async function handleSubmit() {
    ...
    saving.value = true;
    try {
        const docs = stagedDocs.value
            .map((f) => f.file)
            .filter((f): f is File => !!f);
        const photos = stagedPhotos.value
            .map((f) => f.file)
            .filter((f): f is File => !!f);
        emit(
            "saved",
        ...
        >
            ...
            <div class="grid grid-cols-2 gap-3">
                <NFormItem label="เอกสารแนบ">
                    <NUpload
                        multiple
                        :file-list="stagedDocs"
                        :default-upload="false"
                        accept=".pdf,.doc,.docx,.xls,.xlsx"
                        class="upload-full"
                        @change="(d) => (stagedDocs = d.fileList)"
                    >
                        <div
                            class="flex flex-col items-center justify-center gap-1.5 py-4 rounded-xl border border-dashed cursor-pointer transition-all hover:border-emerald-400 hover:bg-emerald-50"
                ...
                <NFormItem label="แนบภาพหน้างาน">
                    <NUpload
                        multiple
                        :file-list="stagedPhotos"
                        :default-upload="false"
                        accept=".jpg,.jpeg,.png,.webp"
                        class="upload-full"
                        @change="(d) => (stagedPhotos = d.fileList)"
                    >
                        <div
                            class="flex flex-col items-center justify-center gap-1.5 py-4 rounded-xl border border-dashed cursor-pointer transition-all hover:border-emerald-400 hover:bg-emerald-50"
                            style="border-color: #d3d1c7; text-align: center"
        ...
        <template #footer>
            <div class="flex gap-2 justify-end">
                ...
                <NButton
                    type="primary"
                    :loading="saving"
                    style="background: #ef9f27; border: none"
                    @click="handleSubmit"
                >
                    <template #icon>
                        <Icon

...
const mainMenu = [
    ...
    ...(isCEO() ? [
        ...
        { label: 'Users', path: '/users', icon: 'mdi:account-group-outline' },
    ...
    <div class="sidebar h-screen bg-[var(--primary)] text-white flex flex-col transition-all duration-300 ease-in-out select-none"
        ...
        <div class="flex-1 overflow-y-auto py-3 px-2 space-y-0.5 scrollbar-none">
            ...
            <div v-if="!isProjectMode()">
                <div v-for="item in mainMenu" :key="item.path"
                    class="nav-item flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer transition-all duration-150 mb-1"
                    :class="route.path === item.path ? 'bg-white/25 shadow-inner' : 'hover:bg-white/15'"
                    :title="isCollapsed ? item.label : ''" @click="go(item.path)">
            ...
            <!-- Project submenu -->
            <div v-else>
                <div class="nav-item flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer hover:bg-white/15 transition-all duration-150 mb-2"
                    :title="isCollapsed ? 'กลับ' : ''" @click="go('/dashboard')">
                    <Icon icon="mdi:arrow-left" width="20" class="shrink-0" />
                    <span v-if="!isCollapsed" class="text-sm font-medium">กลับ</span>
                </div>

                <div class="mx-1 h-px bg-white/10 mb-2" />

                <div v-for="item in projectMenu" :key="item.path"
                    class="nav-item flex items-center gap-3 px-3 py-2.5 rounded-lg cursor-pointer transition-all duration-150 mb-1"
                    :class="isActive(item) ? 'bg-white/25 shadow-inner' : 'hover:bg-white/15'"
                    :title="isCollapsed ? item.label : ''" @click="go(item.path)">
                    <Icon :icon="item.icon" width="20" class="shrink-0" />
                    <span v-if="!isCollapsed" class="text-sm font-medium whitespace-nowrap overflow-hidden">
                        {{ item.label }}
                    </span>
                    <div v-if="isActive(item) && !isCollapsed"
                        class="ml-auto w-1.5 h-1.5 rounded-full bg-white shrink-0" />
                </div>
            </div>
        </div>
            ...
            <div class="flex items-center gap-3 rounded-xl p-2.5 transition-colors hover:bg-white/10 cursor-default"
                ...
                <transition name="fade-slide">
                    <div v-if="!isCollapsed" class="flex-1 overflow-hidden">
                        ...
                        <div class="text-xs opacity-50 truncate capitalize">{{ auth.user?.role || 'Member' }}</div>
                    </div>
                </transition>
                <transition name="fade-slide">
